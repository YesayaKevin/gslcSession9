...
<div class="flex flex-col justify-center items-center min-h-full w-full mt-24">
    <form action="{{ route('login') }}" method="post" class="flex flex-col w-1/4 space-y-4">
        ...
        <div class="flex justify-between">
            <input class="border w-1/5 h-16 pl-4" type="number" name="" id="">
            <input class="border w-1/5 h-16 pl-4" type="number" name="" id="">
            <input class="border w-1/5 h-16 pl-4" type="number" name="" id="">
            <input class="border w-1/5 h-16 pl-4" type="number" name="" id="">
        </div>
        ...
        @if(session()->has('loginError'))
            <div class="alert alert-danger alert-dismissible fade show text-red-500" role="alert">
                {{session('loginError')}}
                <button type="button" class="b  tn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
        @endif --}}
        <a href="{{ route('login') }}" class="text-center w-full bg-yellow-300 rounded-md py-2 hover:font-bold">Verify</a>   
    </form>
</div>
@endsection
